...
// Selectores de Comunidad y Donación
const btnCopiarId = document.getElementById('btn-copiar-id');
const mensajeIdCopiado = document.getElementById('mensaje-id-copiado');
const idGranjero = "58119945-cf53-466a-b576-6ec502494c70";
...
// Lógica de copiar ID de Granjero
btnCopiarId.onclick = () => {
    reproducirClick();
    navigator.clipboard.writeText(idGranjero).then(() => {
        mensajeIdCopiado.style.display = "block";
        setTimeout(() => {
            mensajeIdCopiado.style.display = "none";
        }, 3000); 
    }).catch(err => {
...
// Lógica de copiar Billetera MetaMask
btnDonar.onclick = () => {
    reproducirClick();
